    ...
    <div className="min-h-screen p-6 md:p-10 font-sans relative" style={{ backgroundColor: '#e6f4ea', backgroundImage: 'url("https://www.transparenttextures.com/patterns/absurdity.png")' }} onClick={() => { setIsDeleteMode(false); setSelectedItem(null); }}>
          ...
          <div key={item.id} onClick={(e) => { e.stopPropagation(); if (!isDeleteMode) setSelectedItem(item); }} className={`w-full h-52 rounded-3xl p-5 flex flex-col justify-between shadow-sm border-2 relative transition-all cursor-pointer active:scale-95 ${isDeleteMode ? 'border-red-400 bg-red-50 animate-pulse cursor-default' : 'border-black/5 bg-neutral-100 hover:bg-white'}`}>
            ...
            <div className="mt-2 pt-3 border-t border-black/5 flex flex-col items-center gap-1">
              {(() => {
                ...
                return statusText ? (
                  <span className="text-[10px] font-black text-red-500 italic animate-pulse">
                    {statusText}
                  </span>
                ) : <div className="h-[15px]"></div>; // 表示がない時も高さを保つ
              })()}
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl" onClick={() => setSelectedItem(null)}>
          <div className="relative bg-[#fdf6e3] w-full max-w-lg rounded-[3.5rem] p-10 shadow-2xl text-black border-[10px] border-black overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md" onClick={() => setIsModalOpen(false)}>
          <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl text-black border-4 border-black max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-gray-400"><X size={32} /></button>
            <h2 className="text-3xl font-black mb-6 italic uppercase text-center underline decoration-yellow-400 underline-offset-8">Add New Item</h2>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><label className="text-xs font-black italic ml-2">SHOP</label><input type="text" value={newShop} onChange={(e) => setNewShop(e.target.value)} className="w-full p-4 bg-gray-100 rounded-xl font-bold outline-none border-2 focus:border-black" placeholder="例: ライフ" /></div>
                <div className="space-y-1"><label className="text-xs font-black italic ml-2">PRICE</label><input type="text" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} className="w-full p-4 bg-gray-100 rounded-xl font-bold outline-none border-2 focus:border-black" placeholder="例: 200" /></div>
              </div>
              <div className="space-y-1"><label className="text-xs font-black italic ml-2">MEMO</label><textarea value={newMemo} onChange={(e) => setNewMemo(e.target.value)} className="w-full p-4 bg-gray-100 rounded-xl font-bold outline-none border-2 focus:border-black h-20" placeholder="..." /></div>
              <button className="w-full py-5 bg-black text-white font-black rounded-2xl text-2xl active:scale-95 italic shadow-[0_10px_0_0_rgba(0,0,0,0.2)]" onClick={handleSave}>COMPLETE!</button>
            </div>
          </div>
        </div>
      )}
      ...
      <footer className="fixed bottom-10 right-10 z-[5000] flex flex-col items-end gap-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex gap-4 p-3 bg-white/90 backdrop-blur-md rounded-full shadow-2xl border-4 border-black">
          ...
          <button 
            ...
            className={`p-4 rounded-full transition-all active:scale-75 ${isSortMode ? 'bg-black text-yellow-400' : 'text-black hover:bg-gray-200'}`}
          >
            <ArrowUpDown size={36} strokeWidth={3} />
          </button>
